...
import { Box, Grid, Card, Link, Stack, Button, Divider, Container, Typography, ButtonGroup,  } from '@mui/material';
      ...
      <Container component={MotionViewport}>
        <Box sx={{ mb: 10, textAlign: 'center' }}>
          ...
          <m.div variants={varFade().inDown}>
            <Typography variant="subtitle2" sx={{ mb: 3, color: 'text.disabled', fontWeight: '300' }}>
          ...
          <m.div variants={varFade().inDown}>
            ...
            <Stack direction="row" displayy="flex" justifyContent="center" gap="10px">
...
function PlanCard({ plan }) {
  ...
  const Free = license === 'Free';
  const usd = license === '39 Usd ';
    ...
    >
      <Stack spacing={5}>
        ...
        >
          ...
          <Typography variant="subtitle1" sx={{ background: '#38B2AC', padding: '.5px 15px', borderRadius: '5rem' }}>
            Free
          </Typography>
          <Typography variant="h4" marginY="20px" textTransform="uppercase">
            {' '}
            {license}
          </Typography>
        </div>
        ...
        <Stack spacing={2.5}>
          {commons.map((option) => (
            <Stack key={option} spacing={1.5} direction="row" alignItems="center" justifyContent="space-between">
              <Typography variant="body2">{option}</Typography>
              <CheckCircleIcon icon={'eva:checkmark-fill'} sx={{ color: 'primary.main', width: 20, height: 20 }} />
            </Stack>
          ))}
